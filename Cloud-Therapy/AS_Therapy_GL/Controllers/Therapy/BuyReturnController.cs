...
namespace AS_Therapy_GL.Controllers
{
    public class BuyReturnController : AppController
    {
        ...
        public void Insert_StkTransMst_LogData(PST_TRANSMST model)
        {
        ...
        public void Update_StkTransMst_LogData(PST_TRANSMST model)
        {
            TimeZoneInfo timeZoneInfo;
            timeZoneInfo = TimeZoneInfo.FindSystemTimeZoneById("Central Asia Standard Time");
            DateTime PrintDate = TimeZoneInfo.ConvertTime(DateTime.Now, timeZoneInfo);
        ...
        public void Delete_StkTransMst_LogData(PST_TRANSMST model)
        {
            TimeZoneInfo timeZoneInfo;
            timeZoneInfo = TimeZoneInfo.FindSystemTimeZoneById("Central Asia Standard Time");
            DateTime PrintDate = TimeZoneInfo.ConvertTime(DateTime.Now, timeZoneInfo);
        ...
        public void Delete_STK_TransMst_DELETE(PST_TRANSMST model)
        {
            TimeZoneInfo timeZoneInfo;
            timeZoneInfo = TimeZoneInfo.FindSystemTimeZoneById("Central Asia Standard Time");
            DateTime PrintDate = TimeZoneInfo.ConvertTime(DateTime.Now, timeZoneInfo);
        ...
        public ActionResult IndexPost(PageModel model, string command)
        {
            if (command == "Add")
            {
                ...
                model.pst_Trans.TRANSTP = "IRTB";
                model.pst_Trans.RSID = model.AGL_acchart.ACCOUNTCD;


                var res = db.PST_TransDbSet.Where(a => a.TRANSNO == model.pst_Trans.TRANSNO && a.TRANSTP == "IRTB" && a.TRANSYY == model.pst_Trans.TRANSYY && a.COMPID == compid).Count(a => a.ITEMID == model.pst_Trans.ITEMID) == 0;

                if (res == true)
                {
                    var sid = db.PST_TransDbSet.Where(x => x.TRANSNO == model.pst_Trans.TRANSNO && x.TRANSTP == "IRTB" && x.TRANSYY == model.pst_Trans.TRANSYY && x.COMPID == compid)
                                  .Max(o => o.ITEMSL);
                    var transno_Max = db.PST_TransDbSet.Where(x => x.TRANSYY == model.pst_Trans.TRANSYY && x.TRANSTP == "IRTB" && x.COMPID == compid)
                        .Max(s => s.TRANSNO);
                ...
                else
                {
                    var result = (from n in db.PST_TransDbSet
                                  where n.TRANSNO == model.pst_Trans.TRANSNO &&
                                        ...
                                        n.TRANSYY == model.pst_Trans.TRANSYY && n.TRANSTP == "IRTB"
                                  select new
                                  {
            ...
            else if (command == "Save")
            {
                ...
                model.Pst_Transmst.TRANSTP = "IRTB";
                model.Pst_Transmst.RSID = model.AGL_acchart.ACCOUNTCD;

                Insert_StkTransMst_LogData(model.Pst_Transmst);
            ...
            else if (command == "A4")
            {
                ...
                aPageModel.Pst_Transmst.TRANSTP = "IRTB";
        ...
        {
            PST_TRANS stkTrans = db.PST_TransDbSet.Find(tid);

            db.PST_TransDbSet.Remove(stkTrans);
            db.SaveChanges();

            var result = (from t in db.PST_TransDbSet
                          where t.COMPID == compid && t.TRANSNO == orderNo && t.TRANSYY == Year && t.TRANSTP == "IRTB"
                          select new { t.TRANSYY, t.RSID, t.TRANSDT }
             ).Distinct().ToList();


            foreach (var n in result)
            {
                model.pst_Trans.TRANSYY = n.TRANSYY;
                model.AGL_acchart.ACCOUNTCD = n.RSID;
                TempData["ACCOUNTCD"] = model.AGL_acchart.ACCOUNTCD;
            ...
            {
                var sid = db.PST_TransDbSet.Where(x => x.TRANSNO == model.pst_Trans.TRANSNO && x.TRANSYY == Year && x.TRANSTP == "IRTB" && x.COMPID == compid)
                            .Max(o => o.ITEMSL);
        ...
        public ActionResult EditOrder(PageModel model, string command)
        {
            if (command == "Add")
            {
                ...
                var res = db.PST_TransDbSet.Where(a => a.TRANSDT == model.pst_Trans.TRANSDT && a.TRANSNO == model.pst_Trans.TRANSNO && a.TRANSTP == "IRTB" && a.COMPID == compid).Count(a => a.ITEMID == model.pst_Trans.ITEMID) == 0;
                if (res == true)
                {
                    //Permission Check
                    Int64 loggedUserID = Convert.ToInt64(System.Web.HttpContext.Current.Session["loggedUserID"]);
                    ...
                    var get_TRANSDT_TRANSYY = (from m in db.PST_TransDbSet
                                               where m.COMPID == compid && m.TRANSNO == model.pst_Trans.TRANSNO && m.TRANSTP == "IRTB"
                                               select new { m.TRANSDT, m.TRANSYY }).Distinct().ToList();
                    ...
                    var sid = db.PST_TransDbSet.Where(x => x.TRANSNO == model.pst_Trans.TRANSNO && x.TRANSTP == "IRTB" && x.COMPID == compid)
                               .Max(o => o.ITEMSL);
                    var transno_Max = db.PST_TransDbSet.Where(x => x.TRANSYY == model.pst_Trans.TRANSYY && x.TRANSTP == "IRTB" && x.COMPID == compid)
                        .Max(s => s.TRANSNO);
                    ...
                    else
                    {
                        ...
                        var transMst = (from m in db.PST_TransMstDbSet
                                        where m.COMPID == compid && m.TRANSNO == model.pst_Trans.TRANSNO && m.TRANSDT == model.pst_Trans.TRANSDT && m.TRANSTP == "IRTB"
                                        select new { m.PST_TRANSMST_ID, m.TOTAMT, m.DISCOUNT, m.TOTNET,m.REMARKS }).ToList();
                ...
                else
                {
                    var result = (from n in db.PST_TransDbSet
                                  where n.TRANSNO == model.pst_Trans.TRANSNO &&
                                        n.COMPID == compid &&
                                        n.ITEMID == model.pst_Trans.ITEMID &&
                                        n.TRANSDT == model.pst_Trans.TRANSDT && n.TRANSTP == "IRTB"
                                  select new
                                  {
                                      transPID = n.PST_TRANS_ID,
                                      sl = n.ITEMSL,
            ...
            else if (command == "Save")
            {
                ...
                var findTransNO = (from n in db.PST_TransMstDbSet
                                   where n.TRANSNO == model.pst_Trans.TRANSNO && n.COMPID == model.pst_Trans.COMPID &&
                          n.TRANSYY == model.Pst_Transmst.TRANSYY && n.TRANSTP == "IRTB"
                                   select n).ToList();
                if (findTransNO.Count != 0)
                {
                    foreach (PST_TRANSMST pstTransmst in findTransNO)
                    {
                        ...
                        Update_StkTransMst_LogData(pstTransmst);
                    }
            ...
            else if (command == "search")
            {
                ...
                var transMst = (from m in db.PST_TransMstDbSet
                                where m.TRANSDT == model.pst_Trans.TRANSDT && m.COMPID == compid && m.TRANSNO == model.pst_Trans.TRANSNO && m.TRANSTP == "IRTB"
                                select new { m.PST_TRANSMST_ID, m.TOTAMT, m.DISCOUNT,m.TOTNET, m.AMTCASH, m.AMTCREDIT, m.TOTREF,m.REMARKS, m.RSID }).ToList();
                ...
                var stk_Trans = (from m in db.PST_TransDbSet
                                 where m.TRANSDT == model.pst_Trans.TRANSDT && m.COMPID == compid && m.TRANSNO == model.pst_Trans.TRANSNO && m.TRANSTP == "IRTB"
                                 select new { m.RSID, m.TRANSYY }).ToList();
            ...
            else if (command == "A4")
            {
                ...
                if (findTransNO.Count != 0)
                {
                    foreach (PST_TRANSMST pstTransmst in findTransNO)
                    {
                        ...
                        pstTransmst.DISCOUNT = model.Pst_Transmst.DISCOUNT;
                        pstTransmst.TOTNET = model.Pst_Transmst.TOTNET;
                        pstTransmst.AMTCASH = model.Pst_Transmst.AMTCREDIT;
                        pstTransmst.AMTCREDIT = model.Pst_Transmst.AMTCREDIT;
                        pstTransmst.TOTREF = model.Pst_Transmst.TOTREF;
                        pstTransmst.REMARKS = model.Pst_Transmst.REMARKS;
                        Update_StkTransMst_LogData(pstTransmst);
        ...
        public ActionResult EditOrderDelete(Int64 tid, Int64 orderNo, DateTime Date, Int64 Year, Int64 itemsl, PageModel model)
        {
            ...
            if (result.Count == 0)
            {
                var searchRmsTransMst = (from n in db.PST_TransMstDbSet where n.COMPID == compid && n.TRANSNO == orderNo && n.TRANSDT == Date && n.TRANSYY == Year && n.TRANSTP == "IRTB" select n).ToList();
                PST_TRANSMST pstTransmst = new PST_TRANSMST();
                foreach (var m in searchRmsTransMst)
                {
                    pstTransmst = m;

                }
                if (pstTransmst != null && searchRmsTransMst.Count != 0)
                {
                    Delete_StkTransMst_LogData(pstTransmst);
                    Delete_STK_TransMst_DELETE(pstTransmst);
                    db.PST_TransMstDbSet.Remove(pstTransmst);
                    db.SaveChanges();
                }
            ...
            var sid = db.PST_TransDbSet.Where(x => x.TRANSNO == model.pst_Trans.TRANSNO && x.TRANSDT == Date && x.COMPID == compid && x.TRANSYY == Year && x.TRANSTP == "IRTB").Max(o => o.ITEMSL);
            model.pst_Trans.TRANSNO = orderNo;
            model.pst_Trans.ITEMSL = sid;
            model.pst_Trans.TRANSDT = Date;
            model.pst_Trans.TRANSYY = Year;
